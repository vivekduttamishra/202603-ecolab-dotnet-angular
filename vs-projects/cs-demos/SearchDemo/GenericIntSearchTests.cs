...
public class GenericIntSearchTests
{

    List<int> numbers = new List<int>{2, 11, 18, 19, 17, 93, 57, 31, 28, 52};


    class EvenMatcher : IIntMatcher
    {
    ...
    {
        var evens = SearchHelper.SearchInt(numbers, new EvenMatcher());
        foreach(var value in evens)
            Assert.True(value%2==0);
    }


    class InRange : IIntMatcher
    {
        int min;
        int max;
    ...
    {
        var result = SearchHelper.SearchInt(numbers, new InRange(50,60));
